...
describe('Service detection (filesystem-based)', () => {
  ...
  it('clearLogsFile vacía el log', async () => {
    const logPath = path.join(tmpDir, 'webservdev.log');
    fs.writeFileSync(logPath, 'linea1\nlinea2');
    const ok = await clearLogsFile(fsAdapter, logPath);
    assert.equal(ok, true);
    const content = fs.readFileSync(logPath, 'utf-8');
    assert.equal(content, '');
  });

  it('loadAppConfig lee secciones y valores', async () => {
    writeIni(tmpDir, ` [apache]\nversion=2.4.54\nport=8080\n\n[mysql]\nversion=10.6\n`);
  ...
  it('getServiceBinPath detecta ejecutable directo y en subcarpeta bin', async () => {
  ...
  it('apache requiere php: instalado pero sin php -> dependenciesReady false', async () => {
    writeIni(tmpDir, `[apache]\nversion=2.4.0`);
    const apacheBin = path.join(tmpDir, 'bin', 'apache', '2.4.0');
    ensureDir(apacheBin);
    fs.writeFileSync(path.join(apacheBin, 'httpd.exe'), '');
  ...
  it('apache con php instalado -> dependenciesReady true', async () => {
    writeIni(tmpDir, `[apache]\nversion=2.4.0\n[php]\nversion=8.2.0`);
    const apacheBin = path.join(tmpDir, 'bin', 'apache', '2.4.0');
    ensureDir(apacheBin);
    fs.writeFileSync(path.join(apacheBin, 'httpd.exe'), '');
  ...
  it('apache con php en neutralino/bin detecta dependencia y marca dependenciesReady', async () => {
    writeIni(tmpDir, `[apache]\nversion=2.4.0\n[php]\nversion=8.2.0`);
  ...
  it('detecta versiones de PHP en app, usr y neutralino y selecciona la más reciente', async () => {
    writeIni(tmpDir, `[apache]\nversion=2.4.0`);
    ...
    // PHP en tres ubicaciones con módulo Apache
  ...
  it('nginx detecta instalación y puerto por defecto', async () => {
  ...
  it('redis/memcached/mailpit/mongodb/postgresql se detectan con sus binarios', async () => {
    const specs = [
  ...
  it('incluye logs de mysql en data/ y en la versión', async () => {
    writeIni(tmpDir, `[mysql]\nversion=8.0.30`);
  ...
  it('detecta servicios DB extra (redis) si existen binarios', async () => {
